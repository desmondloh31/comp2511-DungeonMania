...
// playerBuff = BattleStatistics.applyBuff(playerBuff,
// merc.getBattleStatistics());
// }
...
// BattleStatistics.applyBuff(playerBaseStatistics, playerBuff);
...
// return;
// List<BattleRound> rounds = BattleStatistics.battle(playerBattleStatistics,
// enemyBattleStatistics);
...
public class BattleFacade {
    ...
    public void battle(Game game, Player player, Enemy enemy) {
        ...
        if (effectivePotion != null) {
            ...
            for (BattleItem item : player.getPlayerEntities()) {
                if (item instanceof Potion)
                    continue;
                playerBuff = item.applyBuff(playerBuff);
                battleItems.add(item);
        ...
        // List<Mercenary> mercs = game.getMap().getEntities(Mercenary.class);
        List<Mercenary> mercs = game.getGameMercenaries();
        for (Mercenary merc : mercs) {
            if (!merc.isAllied())
                continue;
